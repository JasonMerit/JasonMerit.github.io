...
function draw(gl, theta)
{
    ...
    // View
    let V = lookAt(vec3(0, 0, -4.5), vec3(0, 0, 0), vec3(0.0, 1.0, 0.0));  // eye, at, look_up

    // Perspective projection
    let P = perspective(90.0, 2.0, 0.1, 10.0);  // fovy, aspect (w/h), near, far  (near far are clipping)

    sendMVP(gl, mat4(), V, P);
    gl.uniform1i(gl.v_TexIndexLoc, 0);
    gl.drawArrays(gl.TRIANGLES, 0, gl.n);
    initPlanet(gl, vec3(0.5, 0.0, 0.5), vec3(0.5, 0.5, 0.5), theta, 1);
}
